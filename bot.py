...
async def process_promo_code(message: types.Message, state: FSMContext):
    promo_code = message.text.upper() if message.text.lower() != "пропустити" else None
    await state.update_data(promo_code=promo_code, discount=0)

    if promo_code:
        await message.answer(f"🎟️ Промокод <b>{promo_code}</b> застосовано!", reply_markup=ReplyKeyboardRemove())
    else:
        await message.answer("🎟️ Промокод пропущено.", reply_markup=ReplyKeyboardRemove())

    await state.set_state(OrderForm.review)
    await send_order_to_admin(message, state)
    await message.answer(
        "✅ Ваше замовлення з промокодом надіслано адміну. Очікуйте підтвердження.",
        reply_markup=new_order_kb()
    )
    await state.clear()

@dp.callback_query(lambda c: c.data == "submit_order")
async def submit_order(callback: types.CallbackQuery, state: FSMContext):
    await send_order_to_admin(callback.message, state)
    await callback.message.edit_text("✅ Ваше замовлення надіслано! Очікуйте підтвердження.", reply_markup=new_order_kb())
    await state.clear()
    await callback.answer()

@dp.callback_query(lambda c: c.data == "edit_order")
async def edit_order(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("✏️ Введіть новий опис або надішліть фото заново:", reply_markup=cancel_kb())
    await state.set_state(OrderForm.item)
    await callback.answer()
...
async def cancel_order(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text("❌ Замовлення скасовано. Якщо передумаєш — просто натисни /start 🙂", reply_markup=new_order_kb())
    await callback.answer()


def review_order_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Надіслати замовлення", callback_data="submit_order")],
        [InlineKeyboardButton(text="🎟️ Ввести промокод", callback_data="enter_promo")],
        [InlineKeyboardButton(text="🔁 Редагувати замовлення", callback_data="edit_order")],
...
async def ask_promo(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("🎟️ Введіть промокод або напишіть 'Пропустити':", reply_markup=ReplyKeyboardRemove())
